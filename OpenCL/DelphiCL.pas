...
begin
  ...
  FContext := clCreateContext(PPCL_context_properties(@props[0]), 1, @Device_id, nil, nil, @FStatus);
  {$IFDEF LOGGING}
    WriteLog('clCreateContext: ' + GetString(FStatus) + ';');
  {$ENDIF}
  FStatus := clGetContextInfo(FContext, CL_CONTEXT_NUM_DEVICES , SizeOf(FNumDevices), @FNumDevices, nil);
...
begin
  ...
  region[2] := 1;// Image 2D
  FStatus := clEnqueueReadImage(FCommandQueue, Image.FMem, CL_TRUE, PSizet(@origin), PSizet(@region), 0, 0, Data, 0, nil, {$IFDEF PROFILING}@TimingEvent{$ELSE}nil{$ENDIF});
  {$IFDEF LOGGING}
...
begin
  ...
  region[1] := Height;
  region[2] := 1;// Image 2D
  FStatus := clEnqueueWriteImage(FCommandQueue, Image.FMem, CL_TRUE, PSizet(@origin), PSizet(@region), 0, 0, Data, 0, nil, {$IFDEF PROFILING}@TimingEvent{$ELSE}nil{$ENDIF});
  {$IFDEF LOGGING}
