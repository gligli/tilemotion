...
__kernel void MotionPredict(__global struct xyErr_s * outErrs, __global short * prevDCTs, __global short * lineDCTs, int radius, int screenWidth, int screenHeight, int sy)
{
  ...
  __global short * curPtr = &lineDCTs[sx * cTileDCTSize];
  ...
  outErrs[sx].err = bestErr;
  outErrs[sx].x = bestX - dx;
  outErrs[sx].y = bestY - dy;
}
